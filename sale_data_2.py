...
class SaleData:
    ...
    def __init__(self, cover_path, file_path, plan_name, sku_list=None,  storeORshop='store_city', encoding="utf-8"):
        ...
        self.cover_df = pd.read_csv(cover_path,encoding=encoding, low_memory=False , sep='\t')

        self.cover_df = self.cover_df.rename(columns={self.cover_df.columns[0]:'start_city','覆盖省':'end_province','覆盖城市':'end_city'})
...
def main_flow(plan_name, area_name):
    ...
    for RDC in RDC_list:
        ...
        for sku in list(rdc_sale.columns):
            ...
            stock_df[sku], replenish_df[sku], accuracy_sku.loc[sku, RDC] = inventoryCal.stock_simulation_2(
                ...
                k=service_level, bp=bp, nrtk=nrtk, min_except=1)
        print("开始保存")
        # if not os.path.exists('E:/DepartmentProject/mlk/data/goods_layout_{}/'.format(plan_name)):
        #     os.mkdir('E:/DepartmentProject/mlk/data/goods_layout_{}/'.format(plan_name))
        #     os.mkdir('E:/DepartmentProject/mlk/data/goods_layout_{}/sale/'.format(plan_name))
        ...
        stock_sum += int(stock_df.values.sum())
    ...
    for sku in list(cdc_guangzhou_sale.columns):
        vlt = FAC_CDC_info.set_index(['工厂', 'CDC']).loc[(skudata.get_factory(sku), '杭州'), 'vlt(hour)']
        ...
        cdc_guangzhou_stock_df[sku], cdc_guangzhou_replenish_df[sku], accuracy_sku.loc[
            sku, 'CDC杭州'] = inventoryCal.stock_simulation_2(cdc_guangzhou_sale[sku],
                                                            RDC_period,
                                                            math.ceil(
                                                                vlt / 24),
                                                            k=service_level,
    ...
    cdc_guangzhou_sale_use.dropna().to_csv('../data/goods_layout_{}/sale/CDC_{}.csv'.format(plan_name, '杭州'))
    cdc_guangzhou_stock_df.to_csv('../data/goods_layout_{}/stock/CDC_{}.csv'.format(plan_name, '杭州'))
    cdc_guangzhou_replenish_df.to_csv('../data/goods_layout_{}/repleshment/CDC_{}.csv'.format(plan_name, '杭州'))
    stock_sum += cdc_guangzhou_stock_df.values.sum()
    sale_sum += cdc_guangzhou_sale_use.values.sum()
    # turnover_dict['cdc杭州'] = cdc_guangzhou_stock_df.values.sum() / cdc_guangzhou_sale_use.values.sum()
    # print('{}模拟完成！平均周转天数为：{:.2f}'.format('cdc杭州', turnover_dict['cdc杭州']))

    turnover_dict['总体'] = stock_sum / sale_sum
    writer = pd.ExcelWriter(
        '../data/goods_layout_{}/BP{}补货{}调拨{}服务水平{}NRTk{}.xlsx'.format(plan_name, bp, RDC_period, FDC_period,
                                                                       service_level, nrtk))
...
if __name__ == '__main__':
    ...
    print('商品信息表       sku_info_df read over!')
    skudata = sku_info_1.skuData(file_path='../data/{}/sku信息：东北.xlsx'.format(plan_name))
    ...
    print('商品布局方案:{}'.format(goods_layout))
    print(parameter)
    saledata_toc = SaleData('../data/{}/仓网门店数据1：CDC-RDC.xlsx'.format(plan_name),'../data/{}/订单数据：东北.xlsx'.format(plan_name),storeORshop='store_city')
    turnover_dict = {}
    shop_turnover_dict = {}
    xianhuo_day = pd.DataFrame(index=saledata_toc.get_daterange())
    xianhuo_sku = pd.DataFrame(index=saledata_toc.get_sku())
    ...
    cdc_guangzhou_sale_use = pd.DataFrame(index=cdc_guangzhou_sale.index)
    for sku in list(cdc_guangzhou_sale.columns):
        vlt = FAC_CDC_info.set_index(['工厂', 'CDC']).loc[(skudata.get_factory(sku), '杭州'), 'vlt']
        cdc_guangzhou_sale_use[sku] = cdc_guangzhou_sale[sku]
        cdc_guangzhou_stock_df[sku], cdc_guangzhou_replenish_df[sku], accuracy_sku.loc[
            sku, 'CDC杭州'] = inventoryCal.stock_simulation_2(cdc_guangzhou_sale[sku],
                                                            RDC_period,
